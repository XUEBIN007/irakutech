...
let kitchenGroups = core.kitchenQueueGroups();
assert.strictEqual(kitchenGroups.new.length, 2);
assert.strictEqual(kitchenGroups.preparing.length, 0);
assert.strictEqual(kitchenGroups.done.length, 0);
core.updateOrderStatus(kdsOrder.id, 'preparing');
kitchenGroups = core.kitchenQueueGroups();
assert.strictEqual(kitchenGroups.new.length, 0);
assert.strictEqual(kitchenGroups.preparing.length, 2);
assert.strictEqual(kitchenGroups.done.length, 0);
assert.strictEqual(core.loadStore().orders.find((entry) => entry.id === kdsOrder.id).lines.every((line) => line.status === 'preparing'), true);
